...
export default function NetworksPage() {
    const [hasGraph, setHasGraph] = useState(null); // null = checking
    const graphSrc = dataUrl("network/index.html");

    useEffect(() => {
        let active = true;
        (async () => {
            try {
                const res = await fetch(graphSrc, { method: "HEAD" });
                if (!active) return;
                setHasGraph(res.ok);
            } catch {
                if (active) setHasGraph(false);
            }
        })();
        return () => {
            active = false;
        ...
        <section data-testid="networks-page" className="max-w-6xl mx-auto px-6 lg:px-10 pt-16 lg:pt-24 pb-24">
            <header className="mb-12">
                ...
                <p className="mt-6 max-w-[60ch] font-serif text-lg text-ink-muted leading-relaxed">
                    ...
                    produced in R / Gephi and embedded as an interactive HTML
                    document.
                </p>
            </header>
            ...
            <div className="rule-top rule-bottom" data-testid="network-embed-frame">
                {hasGraph === null ? (
                    ...
                    <Placeholder label="Awaiting upload of the network graph." />
                )}
            </div>

            {hasGraph && (
                <p className="mt-5 font-meta text-sm text-ink-muted italic">
                    Click and drag a node to explore the graph; scroll to zoom.
                </p>
            )}
        </section>
    );
...
function Placeholder({ label }) {
    return (
        ...
        >
            <div className="font-display italic text-3xl text-ink-muted">{label}</div>
            <p className="mt-4 font-meta text-sm text-ink-muted max-w-md leading-relaxed">
                The editor will place an exported interactive HTML file
                (produced in R or Gephi) at{" "}
                <code className="font-meta text-xs">public/network/index.html</code>{" "}
                in the repository. It will appear here in this frame.
            </p>
            <img
